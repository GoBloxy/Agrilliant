...
=======
     ...
     *
     * <p>Idempotent: every call site (initialize, ShellView.setOnShowing,
     * LifecycleService RESUME) is safe because {@link #updateDateTime()},
     * {@link #subscribeLiveSensor()} and the listener-attach in
     * {@link #updateSidebarStatus()} all bail out early when their
     * respective fields are non-null.
     *
     * <p>Wired by {@code ShellView.setOnShowing} (handles the post-logout
     * → re-login case) and by {@code Main}'s Gluon Attach
     * {@code LifecycleService.RESUME} subscription (handles OS-level
     * foregrounding on Android).
     */
    public void startLifecycle() {
        updateDateTime();
        subscribeLiveSensor();
        // updateSidebarStatus's listener-attach branch is gated on
        // activeSensorsListener == null, so calling it again only
        // re-attaches; the DB / "online" reads run again too which is
        // fine — it's a constant-time pair of reads.
        updateSidebarStatus();
    }
